...
func negotiate(spn string) Mechanism {
	return Mechanism{
		...
		Start: func(m *Negotiator) (bool, []byte, interface{}, error) {
			...
			logrus.WithField("ret", ret).Info("init sec context")
			switch ret {
			case sspi.SEC_E_OK:
				return false, ctx, challenge, nil
			case sspi.SEC_I_COMPLETE_NEEDED, sspi.SEC_I_COMPLETE_AND_CONTIUNE:
				return true, ctx, challenge, nil
